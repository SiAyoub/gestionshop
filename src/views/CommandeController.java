    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        ObservableList<Article>  list =  FXCollections.observableArrayList();
          try { 
            ...
            ResultSet rs = cnx.createStatement().executeQuery("SELECT id_article, titre_article, image, description_article, prix FROM article");
            while(rs.next())
            {
        ...
        id.setCellValueFactory(data ->
                {
            int idValue = data.getValue().getid_article();
 ...
 Table.setItems(list);
  ...
  @FXML
    private void sortById(ActionEvent event) {
        ObservableList<Article> sortedList = list.sorted(Comparator.comparing(Article::getid_article));
...
private void Commander(ActionEvent event) throws SQLException {
    ...
    try {
        ...
        String insertQuery = "INSERT INTO Commande (id_article, nombre, addresse_commande, nom_prenom_commande, tel_commande) VALUES (?, ?, ?, ?, ?)";

        PreparedStatement preparedStatement = cnx.prepareStatement(insertQuery);
        preparedStatement.setInt(1, idArticleValue);
        preparedStatement.setString(2, nombreValue);
        preparedStatement.setString(3, addressecValue);
        preparedStatement.setString(4, nomprenomValue);
        preparedStatement.setString(5, ntelValue);
        ...
        if (rowsAffected > 0) {
         ...
         String selectQuery = "SELECT * FROM Commande WHERE id_article = ? AND nombre = ?";
            ...
            if (resultSet.next()) {
                int commandeId = resultSet.getInt("id_commande");
                idc.setText("      " + commandeId);
                idp.setText("      " + idArticleValue);
                qtt.setText("      " + nombreValue);
             
        String selectPrixQuery = "SELECT prix FROM article WHERE id_article = ?";
